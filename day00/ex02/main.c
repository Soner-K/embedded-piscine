...
int main(void)
{
	...
	while (true)
	{
		if (PIND & (1 << PD2)) // if PD2 is high, current flows, so switch1 isnt pressed : D1 should be turned off
			PORTB &= ~(1 << PB0);
		else // current doesn't flow, so it is pressed : D1 should be turned on
			PORTB |= (1 << PB0);
